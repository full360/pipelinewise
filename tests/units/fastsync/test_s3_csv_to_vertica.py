...
from pipelinewise.fastsync.s3_csv_to_vertica import tap_type_to_target_type, sync_table, main_impl

PACKAGE_IN_SCOPE = 'pipelinewise.fastsync.s3_csv_to_vertica'
TAP = 'FastSyncTapS3Csv'
TARGET = 'FastSyncTargetVertica'


# pylint: disable=missing-function-docstring,invalid-name,no-self-use
class S3CsvToVertica(unittest.TestCase):
    """
    Unit tests for fastsync s3 csv to vertica
    """
    def test_tap_type_to_target_type_with_defined_tap_type_returns_equivalent_target_type(self):
        self.assertEqual('NUMERIC', tap_type_to_target_type('number'))
