...
from pipelinewise.fastsync.commons.target_vertica import FastSyncTargetVertica


class FastSyncTargetVerticaMock(FastSyncTargetVertica):
    """
    Mocked FastSyncTargetVertica class
    """
...
class TestFastSyncTargetVertica(TestCase):
    """
    Unit tests for fastsync target vertica
    """

    def setUp(self) -> None:
        """Initialise test FastSyncTargetVertica object"""
        self.vertica = FastSyncTargetVerticaMock(
            connection_config={}, transformation_config={})

    def test_create_schema(self):
        """Validate if create schema queries generated correctly"""
        self.vertica.create_schema('new_schema')
        assert self.vertica.executed_queries == [
            'CREATE SCHEMA IF NOT EXISTS new_schema']

    def test_drop_table(self):
        """Validate if drop table queries generated correctly"""
        self.vertica.drop_table('test_schema', 'test_table')
        self.vertica.drop_table('test_schema', 'test_table', is_temporary=True)
        self.vertica.drop_table('test_schema', 'UPPERCASE_TABLE')
        self.vertica.drop_table(
            'test_schema', 'UPPERCASE_TABLE', is_temporary=True)
        self.vertica.drop_table('test_schema', 'test table with space')
        self.vertica.drop_table(
    ...
    def test_create_table(self):
        ...
        self.vertica.executed_queries = []
        self.vertica.create_table(target_schema='test_schema',
        ...
        self.vertica.create_table(target_schema='test_schema',
                                  ...
                                  columns=['"id" INTEGER',
                                           '"column_with space" VARCHAR'],
                                  primary_key=['"id"'])
        assert self.vertica.executed_queries == [
            'CREATE TABLE IF NOT EXISTS test_schema."table with space" ('
            '"id" integer,"column_with space" varchar,'
            '_sdc_extracted_at timestamp,'
            '_sdc_batched_at timestamp,'
            '_sdc_deleted_at varchar'
            ', PRIMARY KEY ("id"))']
        ...
        assert self.vertica.executed_queries == [
            'CREATE TABLE IF NOT EXISTS test_schema."table with space" ('
            '"id" integer,"num" integer,"column_with space" varchar,'
            '_sdc_extracted_at timestamp,'
            '_sdc_batched_at timestamp,'
            '_sdc_deleted_at varchar'
            ', PRIMARY KEY ("id","num"))']
    ...
    def test_grant_select_on_table(self):
        ...
        self.vertica.executed_queries = []
        self.vertica.grant_select_on_table(target_schema='test_schema',
                                           table_name='test_table',
                                           role='test_role',
                                           is_temporary=False)
        assert self.vertica.executed_queries == [
        ...
        self.vertica.grant_select_on_table(target_schema='test_schema',
                                           ...
                                           is_temporary=False)
        assert self.vertica.executed_queries == [
            'GRANT SELECT ON test_schema."full" TO test_role']
        ...
        self.vertica.grant_select_on_table(target_schema='test_schema',
                                           ...
                                           is_temporary=False)
        assert self.vertica.executed_queries == [
            'GRANT SELECT ON test_schema."table with space and uppercase" TO test_role']

    def test_grant_usage_on_schema(self):
        """Validate if GRANT command generated correctly"""
        self.vertica.executed_queries = []
        self.vertica.grant_usage_on_schema(target_schema='test_schema',
                                           role='test_role')
        assert self.vertica.executed_queries == [
    ...
    def test_grant_select_on_schema(self):
        """Validate if GRANT command generated correctly"""
        self.vertica.executed_queries = []
        self.vertica.grant_select_on_schema(target_schema='test_schema',
                                            role='test_role')
        assert self.vertica.executed_queries == [
            'GRANT SELECT ON ALL TABLES IN SCHEMA test_schema TO test_role']
    ...
    def test_swap_tables(self):
        ...
        self.vertica.executed_queries = []
        self.vertica.swap_tables(schema='test_schema',
                                 table_name='test_table')
        assert self.vertica.executed_queries == [
            'DROP TABLE IF EXISTS test_schema."test_table"',
    ...
    def test_obfuscate_columns_case1(self):
        """
        Test obfuscation where given transformations are empty
        Test should pass with no executed queries
        """
        target_schema = 'my_schema'
        table_name = 'public.my_table'

        self.vertica.transformation_config = {}

        self.vertica.obfuscate_columns(target_schema, table_name)
        self.assertFalse(self.vertica.executed_queries)
    ...
    def test_obfuscate_columns_case4(self):
        ...
        self.vertica.obfuscate_columns(
            target_schema, table_name, is_temporary=True)
